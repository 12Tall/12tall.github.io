<template>
    <div class="flex flex-wrap">
        <el-badge class="m-1">
            <el-button
                size="small"
                :type="'all' == selected_cate ? 'primary' : 'default'"
        ...
        <el-badge class="m-1" v-for="(v, k) in categories" :value="v">
            <el-button
                size="small"
                :type="k == selected_cate ? 'primary' : 'default'"
                @click="onCategorySelected(k)"
                >{{ k }}</el-button
            >
        </el-badge>
    </div>
    ...
    <div>
        <el-timeline>
            <el-timeline-item
                v-for="(_, index) in category_contents.slice(
    ...
    <el-pagination
        ...
        :total="category_contents.length"
...
const category_contents = computed(() => {
    if (selected_cate.value != "all") {
        return data.filter(
            (item) => item.url?.split("/")?.[2] === selected_cate.value,
        );
    }
    return data;
});
