<template>
    <div class="flex flex-wrap">
        <el-badge class="m-1">
            <el-button
                size="small"
                :type="'all' == selected_tag ? 'primary' : 'default'"
                @click="onTagSelected('all')"
                >全部</el-button
            >
        </el-badge>
        ...
        <el-badge class="m-1" v-for="(v, k) in tags" :value="v">
            <el-button
                size="small"
                :type="k == selected_tag ? 'primary' : 'default'"
                @click="onTagSelected(k)"
                >{{ k }}</el-button
            >
        </el-badge>
    </div>
    ...
    <div>
        <el-timeline>
            <el-timeline-item
                v-for="(_, index) in tag_contents.slice(
            ...
            >
                <el-tag size="small">{{
                    moment(_.frontmatter.date).format("YYYY-MM-DD")
                }}</el-tag>
                <el-link class="ml-2" :href="_.url">
                    <el-text class="mx-1" size="large" truncated>
                        {{ _.frontmatter.title }}</el-text
                    ></el-link
                >
            </el-timeline-item>
        </el-timeline>
...
const tag_contents = computed(() => {
    if (selected_tag.value != "all") {
        return data.filter(
            (item) => item.frontmatter.tags?.indexOf(selected_tag.value) > -1,
        );
    }
    return data;
});
